...
def obtener_señal(close_arr: np.ndarray, metodo: str, lookback: int, is_short: bool) -> int:
...
def detectar_regimen(close_arr: np.ndarray, metodo: str, lookback: int, is_short: bool) -> str:
    ma = calcular_ma(close_arr, metodo, lookback)
    spread = close_arr - ma
    ...
    if any(v is None for v in [logL_OU, mu, logL_GBM]):
        return "INDETERMINADO"

    if logL_OU - logL_GBM > 0:
        return "REVERSION"
...
def obtener_data_optimizacion(symbol: str) -> Optional[pd.DataFrame]:
    ...
    lunes_actual = (
        ahora - timedelta(days=ahora.weekday())
    ).replace(
        hour=0,
        minute=0,
        second=0,
...
def construir_velas(symbol: str, vela_min: int, lookback: int) -> Optional[pd.DataFrame]:
    total_m1 = (lookback + 50) * vela_min
    ...
    out = pd.DataFrame()
    out['bid'] = df['bid'].resample(
        rule,
        label='right',
        closed='right'
    ).last()

    out['ask'] = df['ask'].resample(
        rule,
        label='right',
        closed='right'
    ).last()
...
def ejecutar_orden(tipo: int, comentario: str) -> bool:
    ...
    if tick is None:
        return False
    if (
        (not IS_SHORT and tipo == mt5.ORDER_TYPE_BUY)
        or
...
def esperar_hasta_siguiente_vela(vela_min: int) -> None:
    ...
    print(f"[{ts()}]: Durmiendo {sleep_s:0f} segundos.")
...
def main():
    ...
    while True:
        ...
        while True:
            ...
            if len(df_velas) < lookback + 5:
                continue

            current_bar = df_velas.index[-1]
            ...
            if en_señal and not posicion_abierta:
                ...
                if IS_SHORT:
                    ...
                    if regimen == "ALCISTA_GBM":
                        ...
                        velas_reversion = 0
                        if velas_regimen >= N_REGIMEN_CONSEC:
